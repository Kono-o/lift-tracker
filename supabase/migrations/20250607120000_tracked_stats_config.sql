-- Tracked stat editor config: start value + optional target.

alter table public.tracked_stats
  add column if not exists start_value real not null default 0;

alter table public.tracked_stats
  add column if not exists has_target boolean not null default false;

alter table public.tracked_stats
  add column if not exists target_value real;

alter table public.tracked_stats
  drop constraint if exists tracked_stats_start_value_nonneg;

alter table public.tracked_stats
  add constraint tracked_stats_start_value_nonneg check (start_value >= 0);

alter table public.tracked_stats
  drop constraint if exists tracked_stats_target_valid;

alter table public.tracked_stats
  add constraint tracked_stats_target_valid check (
