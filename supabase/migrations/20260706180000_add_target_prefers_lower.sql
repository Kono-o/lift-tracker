-- Add target_prefers_lower boolean to tracked_stats
alter table public.tracked_stats
  add column target_prefers_lower boolean not null default true;

-- Update RPC to include target_prefers_lower in save/return
...
begin
  ...
  loop
    ...
    if v_stat_id is not null and exists (
      ...
      insert into public.tracked_stats (user_id, name, unit, display_order, start_value, has_target, target_value, target_prefers_lower, icon)
      values (uid, v_name, v_unit, ord, v_start_value, v_has_target, v_target_value, v_target_prefers_lower, v_icon)
      returning * into row;
      v_stat_id := row.id;
    end if;
