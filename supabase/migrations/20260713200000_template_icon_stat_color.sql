-- Templates get icons (like stats); stats get colors (like templates).
-- icon: 0–15 lucide set (see src/lib/itemIcons.ts)
-- color: 0–255 HSV spectrum (see src/lib/templateColor.ts)

-- ── templates.icon ──────────────────────────────────────────
alter table public.templates
  add column if not exists icon smallint not null default 0;
...
comment on column public.templates.icon is
  'Lucide icon index 0–15 (shared set with tracked_stats.icon)';

-- ── tracked_stats.color ─────────────────────────────────────
alter table public.tracked_stats
  add column if not exists color smallint not null default 242;

alter table public.tracked_stats drop constraint if exists tracked_stats_color_range;
alter table public.tracked_stats
  add constraint tracked_stats_color_range check (color >= 0 and color <= 255);

-- Expand icon range for stats from 0–9 to 0–15
...
comment on column public.tracked_stats.color is
  '0–255 quantized HSV spectrum index (same mapping as templates.color)';

-- ── save_tracked_stats: include color ───────────────────────
